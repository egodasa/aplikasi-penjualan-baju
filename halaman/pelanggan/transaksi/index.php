<?php
  ...
  $sql = "SELECT a.*, b.ongkir, c.nm_jasa, d.nm_kota, (SELECT SUM(bb.hrg_jual) FROM detail_transaksi aa JOIN barang bb ON aa.kd_barang = bb.kd_barang WHERE aa.kd_transaksi = a.kd_transaksi) + b.ongkir AS total_bayar FROM transaksi a JOIN ongkir b ON a.id_ongkir = b.id_ongkir JOIN jasa_pengiriman c ON b.id_jasa = c.id_jasa JOIN kota d ON b.id_kota = d.id_kota WHERE a.kd_pelanggan = ".$_SESSION['kd_pengguna'];
                              ...
                              <td>
                                <div class="form-group">
                                  ...
                                  ?>
                                    ...
                                    <p>
                                      <a href="konfirmasi.php?kd_transaksi=<?=$d['kd_transaksi']?>" class="btn btn-success btn-sm">Konfirmasi Pembayaran</a>
                                    </p>
                                  <?php
                                    }
                                    else if($d['status_transaksi'] == 'Pembayaran Ditolak')
                                    {
                                  ?>
                                    <p>
                                      <a href="konfirmasi.php?kd_transaksi=<?=$d['kd_transaksi']?>" class="btn btn-success btn-sm">Konfirmasi Pembayaran</a>
